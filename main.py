...
# قواعد بيانات مؤقتة في الذاكرة (يفضل مستقبلاً ربطها بقاعدة بيانات دائمية)
server_settings = {}  # لحفظ الرومات والرولات المسموحة لكل سيرفر
bank_data = {}        # لنظام البنك والاقتصاد
roulette_games = {}   # لإدارة ألعاب الروليت الشغالة حالياً
...
def is_allowed(interaction: discord.Interaction):
    ...
    # إذا لم يتم ضبط الإعدادات بعد، مسموح للجميع مؤقتاً أو للإدارة
    ...
    # تحقق من الرول (إذا كان العضو يملك أحد الرولات المسموحة أو إدارة)
    if interaction.user.guild_permissions.administrator:
        return True
        
    if settings.get('roles'):
        user_roles = [role.id for role in interaction.user.roles]
        if not any(role_id in settings['roles'] for role_id in settings['roles']):
            return False
...
# ==================== نظام الروليت (الفعاليات حقت السيرفرات الكبيرة) ====================
...
async def create_roulette(interaction: discord.Interaction, الحد_الاقصى: int = 10, عند_كم_تبدأ: int = 4):
    ...
    roulette_games[channel_id] = {
        ...
        'status': 'انتظار',
        'mode': 'عادي' # عادي = المختار يطرد، عكسي = المختار يفوز
    }
...
@bot.tree.command(name="روليت_عكسي", description="تحويل نظام الروليت الحالي لعكسي (اللي تجي عليه الروليت يعيش/يفوز)")
...
async def join_roulette(interaction: discord.Interaction):
    ...
    # بدء اللعبة تلقائياً إذا وصل العدد المطلوب
...
async def start_roulette_logic(channel, game):
    ...
    while len(players) > 1:
        ...
        if mode == 'عادي':
            ...
            # في النمط العكسي نقدر نعتبره فائز مباشرة أو يستمر لين نطلع الترتيب، هنا بنخليه ينجو والكل يصفق له
    ...
    # تنظيف الروم للجيم القادم
    if channel.id in roulette_games:
        del roulette_games[channel.id]


# ==================== نظام البنك والاقتصاد الاحترافي ====================

def get_bank(user_id):
    if user_id not in bank_data:
        bank_data[user_id] = {'cash': 500, 'bank': 0} # كاش هدية بداية
...
async def daily_salary(interaction: discord.Interaction):
    ...
    # تحديد كمية الكاش بناء على الصعوبة المحددة في السيرفر
...
@bot.tree.command(name="ايداع", description="حفظ الكاش حقك داخل البنك عشان ما ينسرق")
